...
namespace EnergyComparer.Utils
{
    ...
    {
        public static IOperatingSystemAdapter InitializeAdapterService(ILogger logger, bool isProd, bool shouldRestart, IHardwareMonitorService hardwareMonitorServic, IDutAdapter idutAdapter)
        {
            if (Constants.Os == "Win32NT")
            {
                logger.Information("Executing on a {os} machine, the {name} is used.", Constants.Os, "WindowsAdapter");
                return new WindowsAdapter(logger, isProd, shouldRestart, hardwareMonitorServic,idutAdapter);
            }
            else
            {
                logger.Information("Executing on a {os} machine, the {name} is used.", Constants.Os, "LinuxAdapter");
                return new LinuxAdapter(logger,idutAdapter);
            }
        }
